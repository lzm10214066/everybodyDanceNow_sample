...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, label, label_pre=None, image=None, image_pre=None, bg_image=None, infer=False):
        ...
        # Fake Generation
        zero_t = torch.zeros(real_image.size()).data.cuda()
        ...
        if not self.opt.no_vgg_loss:
            loss_G_VGG = self.criterionVGG(fake_image, real_image) * self.opt.lambda_vgg + \
                         self.criterionVGG(fake_pre_image, real_pre_image) * self.opt.lambda_vgg
            loss_G_VGG /= 2.
    ...
    # def forward(self, label, label_pre=None, image=None, image_pre=None, bg_image=None,infer=False):
    #     # Encode Inputs
    #     input_label, input_pre_label, real_image, real_pre_image = self.encode_input(label, label_pre, image, image_pre,bg_image)
    #
    #     # Fake Generation
    #     zero_t=torch.zeros(real_image.size()).data.cuda()
    #     input_pre_concat=torch.cat((input_pre_label,zero_t),dim=1)
    ...
    #
    #     # Fake Detection and Loss
    #     input_label_concat=torch.cat((input_label,input_pre_label),dim=1)
    #     fake_image_concat=torch.cat((fake_image,fake_pre_image),dim=1)
    #     pred_fake_pool = self.discriminate(input_label_concat, fake_image_concat, use_pool=True)
    #     loss_D_fake = self.criterionGAN(pred_fake_pool, False)
    #
    #     # Real Detection and Loss
    #     real_image_concat=torch.cat((real_image,real_pre_image),dim=1)
    #     pred_real = self.discriminate(input_label_concat, real_image_concat)
    #     loss_D_real = self.criterionGAN(pred_real, True)
    #
    #     # GAN loss (Fake Passability Loss)
    #     pred_fake = self.netD.forward(torch.cat((input_label_concat, fake_image_concat), dim=1))
    #     loss_G_GAN = self.criterionGAN(pred_fake, True)
    #
    #     # GAN feature matching loss
    #     loss_G_GAN_Feat = 0
    ...
    #
    #     # Only return the fake_B image if necessary to save BW
    #     return [ self.loss_filter( loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake ),fake_image,fake_pre_image ]

    def inference(self, label, label_pre=None,bg_image=None):
        # Encode Inputs        
        input_label, input_pre_label, _, _ = self.encode_input(Variable(label), Variable(label_pre),
                                                               real_image=None, real_pre_image=None,
                                                               bg_image=Variable(bg_image),infer=True)
...
class InferenceModel(Pix2PixHDModel):
    def forward(self, inp):
        label, label_pre,bg = inp
        return self.inference(label, label_pre,bg)